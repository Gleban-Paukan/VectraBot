...
PHOTO_DIR = '/home/vectra_telegram_bot/project_root/user_data/user_photos'
...
photos_dict = {}
user_states = {}
designers = [1223719258, 1125076741]  # 1223719258, 1493818085, 1984991753
...
def start_message_handler(message):
    ...
    try:
        if len(command_text.split()) > 1 and user.lead_id is not None:
            if not ("order" in message.text):
                return
            command = command_text.split()[1]
            ...
            status = order_data[5]
            if status != "WAITING":
                bot.send_message(message.chat.id, "К сожалению, этот заказ уже выполняет другой исполнитель")
                return
...
Номер телефона: <b>{user.phone_number}</b>
    """
            ...
            markup.add(
                types.InlineKeyboardButton("Подтвердить заказ",
                                           callback_data=f"confirm_order+/+{message.chat.id}+/+{order_id}"))
            for admin_id in data_base_functions.get_admins_list():
                admin_bot.send_message(admin_id[0], text, reply_markup=markup)

    except Exception as er:
        print("An exception in start message", er)
        bot.send_message(message.chat.id, "Неправильный дополнительный код.")
...
def text_message_handler(message):
    if message.text == "🎁 Программа лояльности":
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        user = data_base_functions.SQLiteUser(message.chat.id)
        if user.manager != "SEARCHING" and user.manager is not None:
            markup.add(types.InlineKeyboardButton("Менеджер", url="https://t.me/+79370602463"),
                       types.InlineKeyboardButton("Звонок на квалификатора", callback_data="call_to_manager"))
        else:
            markup.add(types.InlineKeyboardButton("Менеджер", callback_data="send_request_for_manager_searching"),
        ...
        bot.send_message(message.chat.id, "Выберите способы связи:", reply_markup=markup)
        ...
        if user.manager is None:
            ...
            bot.send_message(message.chat.id,
                             ...
                             reply_markup=markup)
    update_every_day_job(message.chat.id)
...
def inline_handler(call):
    ...
    if call.data == "wanna_know_more_montage":
        ...
        markup.add(types.InlineKeyboardButton("Да", callback_data="example_message"),
                   types.InlineKeyboardButton("Нет", callback_data="finish_of_example_message"))
    ...
    elif call.data == "call_to_manager":
        bot.send_message(call.message.chat.id,
                         "Номер телефона вашего менеджера: <b>+79370602463, Мария</b>")  # TODO ADD MANAGERS TO THE SEPARATED LIST
        ...
        bot.send_message(call.message.chat.id, f"На данный момент у вас на балансе <b>{user.balance}</b>",
                         reply_markup=config.start_markup())
    elif call.data == "call_to_qualifier":
        bot.send_message(call.message.chat.id, "<b>Звонок бесплатный по всей России: +78006003920</b>",
                         reply_markup=config.start_markup())
    elif call.data == "send_request_for_manager_searching":
        user = data_base_functions.SQLiteUser(call.message.chat.id)
        if user.manager is None:
            schedule_manager(call.message.chat.id)
            bot.send_message(call.message.chat.id, "Пока у вас нет менеджера, но я зафиксировала вашу заявку!")
            typing_action(call.message.chat.id, 2)
            bot.send_message(call.message.chat.id,
                             "Подберем для вас самого лучшего менеджера и обязательно вас познакомим!"
                             "\n\nОбычно это занимает не более 3 часов ⏰")
            user.change_manager("SEARCHING")
        elif user.manager == "SEARCHING":
            bot.send_message(call.message.chat.id,
...
📏Объем: {square} м²           
        ...
        if value == "job_categories":
            ...
            markup.add("Назад в меню")
            msg = bot.send_message(call.message.chat.id, "Введите новую география работ:", reply_markup=markup)
...
"""
        ...
        markup.add(
            types.InlineKeyboardButton("Подтвердить заказ",
                                       callback_data=f"confirm_order+/+{call.message.chat.id}+/+{order_id}"))
        for admin_id in data_base_functions.get_admins_list():
            admin_bot.send_message(admin_id[0], text, reply_markup=markup)
        ...
        buttons = [
            ...
            types.InlineKeyboardButton(f"{user.define_check_mark('site')} Указать сайт", callback_data="website"),
            types.InlineKeyboardButton(f"{user.define_check_mark('email')} Указать почту", callback_data="email"),
...
"""
    ...
    update_every_day_job(call.message.chat.id)
...
def change_city_name(msg):
    if msg.text == "Назад в меню":
        bot.send_message(msg.chat.id, "Возвращено в меню", reply_markup=config.start_markup())
        ...
        if msg.text == "Вся Россия":
            ...
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            bot.send_message(msg.chat.id, "Данные сохранены.", reply_markup=config.start_markup())
            msg_to_profile(msg.chat.id)
            return
        user = data_base_functions.SQLiteUser(msg.chat.id)
...
def update_job_interval(job_name, new_time):
    ...
    if job:
        try:
            ...
            print(f"Ошибка при обновлении job '{job_name}': {str(e)}")


def update_every_day_job(user_id):
    update_job_interval(f"{user_id}_register_confirmation_1_remind_second_bot", 2880)
    update_job_interval(f"{user_id}_register_confirmation_2_remind_second_bot", 7200)
    update_job_interval(f"{user_id}_register_confirmation_3_remind_second_bot", 1080)
